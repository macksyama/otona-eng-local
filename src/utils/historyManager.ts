import { LessonHistory, saveHistory as saveFirestoreHistory, getHistories as getFirestoreHistories, deleteHistory as deleteFirestoreHistory } from './firestoreHistory';
...
// 履歴を保存
export async function saveHistory(history: LessonHistory) {
  if (isGoogleLogin()) {
...
// 履歴一覧を取得
export async function getHistories(): Promise<LessonHistory[]> {
  if (isGoogleLogin()) {
...
export async function deleteHistory(lessonId: string) {
  if (isGoogleLogin()) {
