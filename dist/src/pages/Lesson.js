...
// 先生アイコン（リアル写真・相対パス）
const TeacherIcon = (_jsx("img", { src: "./teacher-avatar.png", alt: "Teacher", className: "w-10 h-10 rounded-full object-cover border border-blue-200 bg-white", draggable: false }));
// ユーザーアイコン（user.png画像）
const UserIcon = (_jsx("img", { src: "./user.png", alt: "User", className: "w-10 h-10 rounded-full object-cover border border-gray-300 bg-white", draggable: false }));
// AIリクエスト共通関数
async function askAIWeb(prompt) {
    try {
...
function parseAIResponse(raw) {
    try {
        ...
        if (!msg)
            return { error: 'AI応答のmessageが空', raw };
...
const Lesson = ({ article, setPage, setSummaryData }) => {
    ...
    const [discussionHistory, setDiscussionHistory] = useState({ user1: '', aiFollow: '', user2: '' });
    // サマリー生成ボタン押下時の処理
    ...
    return (_jsxs("div", { className: "flex flex-col h-screen", children: [_jsxs("div", { className: "flex-1 overflow-auto p-4 border-b bg-white", children: [_jsx("h2", { className: "font-bold mb-2", children: "\u5165\u529B\u8A18\u4E8B" }), _jsx("div", { className: "whitespace-pre-wrap text-gray-800", children: article })] }), _jsxs("div", { className: "flex-1 overflow-auto p-4 bg-gray-100 flex flex-col", children: [_jsxs("div", { className: "flex-1 overflow-y-auto mb-2", children: [chat.map((msg, i) => (_jsxs("div", { className: "mb-2", children: [msg.role === 'ai' && (_jsxs("div", { className: "flex items-start", children: [_jsx("div", { className: "mr-2 flex-shrink-0", children: TeacherIcon }), _jsxs("div", { className: "bg-white rounded-lg px-4 py-2 shadow text-blue-700 font-bold max-w-[70%]", children: ["AI: ", msg.text] })] })), msg.role === 'user' && (_jsxs("div", { className: "flex items-start justify-end", children: [_jsxs("div", { className: "bg-blue-600 text-white rounded-lg px-4 py-2 shadow max-w-[70%] text-right", children: ["\u3042\u306A\u305F: ", msg.text] }), _jsx("div", { className: "ml-2 flex-shrink-0", children: UserIcon })] })), msg.role === 'ai-feedback' && (_jsxs("div", { className: "flex items-start", children: [_jsx("div", { className: "mr-2 flex-shrink-0", children: TeacherIcon }), _jsx("div", { className: "bg-green-50 rounded-lg px-4 py-2 shadow text-green-700 max-w-[70%]", children: typeof msg.score === 'number' && (msg.modelAnswer !== undefined || msg.mistakes !== undefined || msg.correction !== undefined || msg.advice !== undefined) && !msg.text?.startsWith('AI応答の解析に失敗しました') ? (_jsxs("div", { children: [_jsxs("div", { children: [_jsx("b", { children: "\u30B9\u30B3\u30A2:" }), " ", msg.score, "\u70B9"] }), (msg.mistakes ?? '') !== '' && _jsxs("div", { children: [_jsx("b", { children: "\u9593\u9055\u3044:" }), " ", msg.mistakes] }), (msg.correction ?? '') !== '' && _jsxs("div", { children: [_jsx("b", { children: "\u4FEE\u6B63\u4F8B:" }), " ", msg.correction] }), (msg.advice ?? '') !== '' && _jsxs("div", { children: [_jsx("b", { children: "\u30A2\u30C9\u30D0\u30A4\u30B9:" }), " ", msg.advice] }), (msg.modelAnswer ?? '') !== '' && _jsxs("div", { children: [_jsx("b", { children: "\u6A21\u7BC4\u89E3\u7B54:" }), " ", msg.modelAnswer] })] })) : (_jsxs("div", { children: ["AI\u8A55\u4FA1: ", msg.text, " ", _jsxs("span", { className: "ml-2 font-bold", children: ["+", msg.score, "\u70B9"] })] })) })] }))] }, i))), error && _jsx("div", { className: "text-red-600 whitespace-pre-wrap", children: error })] }), !showResult && currentQuestion && (_jsxs("div", { className: "flex flex-col", children: [!(currentQuestion.type === 'discussion' && discussionSubStep === 2) && (_jsxs(_Fragment, { children: [_jsx("textarea", { className: "w-full h-24 p-2 border rounded mb-2", value: input, onChange: e => setInput(e.target.value), placeholder: "\u3042\u306A\u305F\u306E\u56DE\u7B54\u3092\u5165\u529B\u3057\u3066\u304F\u3060\u3055\u3044", disabled: loading }), _jsx("button", { className: "bg-blue-600 text-white px-4 py-2 rounded self-end disabled:opacity-50", onClick: handleSend, disabled: !input.trim() || loading || !currentQuestion || !currentQuestion.text, children: loading ? '送信中...' : '回答送信' })] })), _jsxs("div", { className: "mt-2 text-sm text-gray-500", children: [step + 1, " / ", totalQuestions, " \u554F"] }), currentQuestion.type === 'discussion' && discussionSubStep === 2 && (_jsx("button", { className: "mt-4 px-4 py-2 bg-green-600 text-white rounded", onClick: handleSummary, disabled: summaryLoading, children: summaryLoading ? 'サマリー生成中...' : 'Summary' }))] }))] })] }));
};
